...
public:
   ...
   void setFirstName(string newName) {firstName_ = newName;};
...
//man's middle name is the first name of his father, woman's middle name is the first name of her mother
class NameChanger: public PersonVisitor{
   public:
      void visit(Man *m) override { setName(m); }
      void visit(Woman *w) override { setName(w); }
   private:
      void setName(Man* m) {
         string temp = m->getFirstName();
         Person* father = m->getFather();
         if (father) {
            temp += " "+father->getFirstName().substr(0, father->getFirstName().find(' '));
         }
         m->setFirstName(temp);
      };
      void setName(Woman* w) {
         string temp = w->getFirstName();
         Person* mother = w->getMother();
         if (mother) {
            temp += " "+mother->getFirstName().substr(0, mother->getFirstName().find(' '));
         }
         w->setFirstName(temp);
      };
};
