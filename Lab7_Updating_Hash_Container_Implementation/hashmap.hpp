...
public:
   ...
   pair<pair<const Key, Value>*, bool> erase(const Key& x);         // erases element with key x, if exists
   Value& operator[] (const Key& x); // returns reference on value of
                                     // element with key, inserts if does not exist

   void rehash(size_t n);            // rehashes the container to have n buckets
private:
...
template <typename Key, typename Value, typename Compare, typename Hash>
pair<pair<const Key, Value>*, bool> hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
   ...
   if (it == elems_[bucket].end()) {
      isInserted = true;
      ++size_;
      elems_[bucket].emplace_back(x);
      it = --elems_[bucket].end();
   } 

   return make_pair(&(*it), isInserted);
}
...
pair<pair<const Key, Value>*, bool> hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
   bool isErased = false;
   typename list<pair<const Key, Value>>::iterator nextElement;
   ...
   if (it != elems_[bucket].end()) {    // the element exists, erase it
      isErased = true;
      nextElement = elems_[bucket].erase(it);
      --size_;

      // If the next element is at the end of the current bucket, find the next non-empty bucket
      if (nextElement == elems_[bucket].end()) {
         for (size_t i = bucket + 1; i < elems_.size(); ++i) {
            if (!elems_[i].empty()) {
               nextElement = elems_[i].begin();
               break;
            }
         }
      }
   ...
   return make_pair(nextElement == elems_[bucket].end() ? nullptr : &(*nextElement), isErased);
}
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
   ...
   // pair<const pair<const Key, Value>*, bool> result = insert(make_pair(x, Value()));
...
// implement void rehash(size_t n) sets the number of buckets in the container to n.
// Note that the existing elements need to be re-arranged according to their new hash value.
// Note also that Hash: the object that provides hashing, needs to be updated to contain the new number of buckets.
// If the parameter n is smaller than the current number of buckets, no actions need to be taken.

template <typename Key, typename Value, typename Compare, typename Hash>
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n) {
   size_t oldNumBuckets = hash_.numBuckets();
   if (n <= oldNumBuckets) {
      return;
   }

   // Create a new hash object with n buckets
   auto oldHashmap = std::move(elems_);

   elems_.clear();
   elems_.resize(n);
   size_ = 0;

   for(const auto& bucket : oldHashmap) {
